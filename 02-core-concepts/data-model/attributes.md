...
|--------|------|-------------|
| entity_type | enum | DEVICE, ASSET, etc. |
| entity_id | UUID | Entity identifier |
| attribute_type | enum | Scope (1=CLIENT, 2=SERVER, 3=SHARED) |
| attribute_key | string | Attribute name |
| bool_v | boolean | Boolean value (if applicable) |
| long_v | bigint | Long value (if applicable) |
| dbl_v | double | Double value (if applicable) |
| str_v | varchar | String value (if applicable) |
| json_v | text | JSON value (if applicable) |
| last_update_ts | bigint | Last modification timestamp |

### Index Strategy

- Primary: (entity_type, entity_id, attribute_type, attribute_key)
