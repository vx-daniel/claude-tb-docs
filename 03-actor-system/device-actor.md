...
graph TB
    subgraph "Device Actor State"
        SM[Sessions Map<br/>Max: 100 sessions]
        SM --> S1[Session 1<br/>ID, Type, Node, LastActivity]
        SM --> S2[Session 2<br/>ID, Type, Node, LastActivity]
        SM --> SN[Session N<br/>...]
    end

    subgraph "Subscription State"
        AS[Attribute Subscriptions<br/>Session → Subscribed]
        RS[RPC Subscriptions<br/>Session → Subscribed]
    end
```
...
- **Max Concurrent Sessions**: 100 per device (configurable)
- **Eviction Policy**: Oldest session removed when limit exceeded
- **Inactivity Timeout**: Sessions closed after 10 minutes without activity (configurable)
...
graph TB
    subgraph "Pending RPC Map"
        P1[Request 1<br/>ID, Request, Sent, Acked, Timeout]
        P2[Request 2<br/>ID, Request, Sent, Acked, Timeout]
        PN[Request N<br/>...]
    end
```

Each pending RPC tracks:
- **Request ID**: Sequential identifier
