...
TbContext provides access to platform services for data operations.
...
| `tellSelf(msg, delay)` | Schedule self-delivery | Delayed processing |

### Nested Rule Chain Methods

| Method | Description |
|--------|-------------|
| `input(msg, chainId)` | Enter nested rule chain |
...
| `transformMsg(orig, type, originator, metadata, data)` | Transform existing message |
...
For nodes that execute user-defined scripts:
